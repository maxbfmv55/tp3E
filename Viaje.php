...
class Viaje
{
    ...
    { //no es necesario inicializar la coleccion de pasajeros 
        $this->codigo = $codigo;
    ...
    {
        $agregaP = false;
        $rtaPasajeros = $this->getPasajeros();
        if (count($rtaPasajeros) < $this->getCantMaxP()) {
            $indice = $this->verificaExistencia($pasajero->getDocumento());

            if ($indice == -1) {
                $rtaPasajeros[] = $pasajero;
                $this->setPasajeros($rtaPasajeros);
                $agregaP = true;
            }
        }
        return $agregaP;
    ...
    public function agregarResponsableV($objResponsableV)
    {
        ...
        */
        $rtaResponsable = $this->responsableV;
        if ($rtaResponsable and ($rtaResponsable == $objResponsableV->getNroEmpleado())) {
            $rta = false;
        } else {
